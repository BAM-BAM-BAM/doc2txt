...
class TestInvSupportedExtensions:
    """Test the SUPPORTED_EXTENSIONS constant."""

    def test_inv_contains_pdf(self):
        assert '.pdf' in SUPPORTED_EXTENSIONS

    def test_inv_contains_docx(self):
        assert '.docx' in SUPPORTED_EXTENSIONS

    def test_inv_contains_doc(self):
        assert '.doc' in SUPPORTED_EXTENSIONS

    def test_inv_contains_rtf(self):
        assert '.rtf' in SUPPORTED_EXTENSIONS

    def test_inv_contains_odt(self):
        assert '.odt' in SUPPORTED_EXTENSIONS


class TestIntFindDocuments:
    """Test find_documents() with multiple formats."""

    def test_int_finds_pdf_files(self, tmp_path):
    ...
    def test_int_finds_docx_files(self, tmp_path):
        (tmp_path / "test.docx").touch()
        result = find_documents(tmp_path, quiet=True)
        assert len(result) == 1
        assert result[0].suffix == '.docx'

    def test_int_finds_mixed_formats(self, tmp_path):
    ...
    def test_int_format_filter(self, tmp_path):
        (tmp_path / "a.pdf").touch()
        (tmp_path / "b.docx").touch()
    ...
    def test_int_recursive_finds_nested(self, tmp_path):
        sub = tmp_path / "sub"
        sub.mkdir()
    ...
    def test_bound_case_insensitive(self, tmp_path):
        (tmp_path / "test.DOCX").touch()
        result = find_documents(tmp_path, quiet=True)
        assert len(result) == 1

    def test_bound_empty_directory(self, tmp_path):
        result = find_documents(tmp_path, quiet=True)
        assert len(result) == 0

    def test_int_shuffle(self, tmp_path):
        for i in range(20):
...
class TestEvalHeadingLevel:
    """Test _heading_level() helper."""

    def test_eval_heading_1(self):
        assert _heading_level("Heading 1") == 1

    def test_eval_heading_2(self):
        assert _heading_level("Heading 2") == 2

    def test_eval_heading_3(self):
        assert _heading_level("Heading 3") == 3

    def test_eval_no_number_defaults_to_1(self):
        assert _heading_level("Heading") == 1


class TestQualExtractTextFromDocx:
    ...
    def test_qual_basic_extraction(self, tmp_path):
    ...
    def test_bound_empty_docx(self, tmp_path):
        path = self._create_docx(tmp_path)
        result = extract_text_from_docx(path)
        assert len(result) == 1
        assert result[0] == ''

    def test_qual_heading_conversion(self, tmp_path):
        import docx
        doc = docx.Document()
    ...
    def test_qual_table_extraction(self, tmp_path):
        import docx
        doc = docx.Document()
    ...
    def test_qual_page_break_splits_sections(self, tmp_path):
        import docx
...
class TestContractExtractTextDispatcher:
    """Test the extract_text() format dispatcher."""

    def test_contract_dispatches_docx(self, tmp_path):
        import docx
        doc = docx.Document()
    ...
    def test_bound_unsupported_format_raises(self, tmp_path):
...
class TestIntProcessDocument:
    """Test process_document() with DOCX files."""

    def test_int_creates_md_from_docx(self, tmp_path):
        import docx
        doc = docx.Document()
    ...
    def test_int_skip_existing_md(self, tmp_path):
        import docx
        doc = docx.Document()
    ...
    def test_int_overwrite_existing_md(self, tmp_path):
        import docx
        doc = docx.Document()
    ...
    def test_int_dry_run(self, tmp_path):
        import docx
        doc = docx.Document()
        doc.add_paragraph("Content")
        path = tmp_path / "test.docx"
